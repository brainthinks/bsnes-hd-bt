auto PPU::Line::renderObject(PPU::IO::Object& self) -> void {
  ...
  for(uint x : range(256)) {
    ...
    if(self.aboveEnable && !windowAbove[x]) plotAbove(x, source, priority[x], decode(cgram[palette[x]]));
    if(self.belowEnable && !windowBelow[x]) plotBelow(x, source, priority[x], decode(cgram[palette[x]]));
  }
}
